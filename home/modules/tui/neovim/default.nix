with builtins;
  instance: {
    ...
    pluginsDir = ./plugins;
    pluginPaths = readDir ./plugins;
    toAbsolutePath = relative: toString pluginsDir + "/" + relative;
    removeLuaExt = path: replaceStrings [".lua"] [""] path;

    luaPathToPlugin = path: let
      pluginName = removeLuaExt relativePath;
      absolutePath = toAbsolutePath relativePath;
    in {
      plugin = getAttr pluginName (pkgs.vimPlugins // customPlugins);
      config = inlineLuaFile absolutePath;
    };

    nixPathToPlugin

    pluginsFromDir = mapAttrs pathToPlugin pluginPaths;
  in
    with keyboard; {
